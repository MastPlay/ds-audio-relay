...
async def on_ready():
    ...
    # Проверим наличие метода listen у VoiceClient (через глобальный класс)
    vc = discord.VoiceClient  # в новых версиях доступен так
    logger.info(f"Есть ли метод listen у VoiceClient? {'listen' in dir(vc)}")
    if 'listen' in dir(vc):
